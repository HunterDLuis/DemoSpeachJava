package com.demospeachjava.service;
...
import com.demospeachjava.activity.configure.Say;
...
//this allows us to make a notification in the top bar
    private Notification buildForegroundNotification(String filename) {
        ...
        Intent resultIntent = new Intent(this, Say.class);
...
//this is when the start button in the activity_finder is pressed
    ...
    public int onStartCommand (Intent intent, int flags, int startId)
    {
        ...
        //Palabra palabra = dbHelper.getPhraseKey();
        //String keyPhrase = palabra.getPalabra().toString();
        //initialize the thing that will keep the service running
        location = (String) intent.getExtras().get("coordenadas");
        direccion = (String) intent.getExtras().get("direccion");
...
//here is what we use to trigger the beeping and vibrating
            ...
            if (result.toLowerCase().contains(ayuda.toLowerCase())){
...
//if the sound is loaded and ready to go then we execute the beeping noise below
                ...
                //send sms with location position of the users
       ...
       try{
            ...
            if (permisionCheck != PackageManager.PERMISSION_GRANTED){
                ...
                ActivityCompat.requestPermissions((Activity) getApplicationContext(),
                        new String[]{Manifest.permission.SEND_SMS},225);
            }else{
                Log.i("Mensaje", "Se tiene los permisos para enviar SMS");
            }
            SmsManager mensaje = SmsManager.getDefault();
            mensaje.sendTextMessage(numPhone, null, "Necesito ayuda \n"+direccion+" "+location, null, null);
            //Toast.makeText(getApplicationContext(), "Necesito ayuda mis coordenadas son \n"+direccion+" "+location, Toast.LENGTH_SHORT).show();
        }catch (Exception e){
            Toast.makeText(getApplicationContext(), "Mensaje no enviado verifique permisos o datos",
                    Toast.LENGTH_SHORT).show();
        }
    }
}
